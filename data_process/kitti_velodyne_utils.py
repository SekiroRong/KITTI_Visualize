# -*- coding = utf-8 -*-
# @Time : 28/10/2022 下午5:00
# @Author : 戎昱
# @File : kitti_velodyne_utils.py
# @Software : PyCharm
...
def makeFVMap(PointCloud_, boundary):
    Height = boundary[0]
    Width = boundary[1]
...
def filter_fov_lidar(lidar):
    mask = np.where((lidar[:, 0] > lidar[:, 1]) & (lidar[:, 0] > -lidar[:, 1]))
    lidar = lidar[mask]
    return lidar
